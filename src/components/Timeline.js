...
import ChannelsImages from './timeline/ChannelsImages'
import TimeTimeline from './timeline/TimeTimeline'
...
export default () => {
  ...
  const scrollToNowLine = (smooth=true) => {
    if (nowRef && nowRef.current) {
      scrollTo(nowRef.current.parentElement, parseInt(getComputedStyle(nowRef.current).left) - window.innerWidth/4, 0, smooth)
    }
  }

  const computeNowLineLeft = () => {
    const now = new Date()
    const hours = now.getHours() + now.getMinutes() * 100 / 6000
  ...
  return (
    <div className="border-top timeline-container">
      {loading && <Spinner />}
      <button className="now-button" onClick={scrollToNowLine} ref={nowButtonRef}>Now</button>
      ...
      <div className="timeline-channels" style={{height: `calc(${channels.length} * var(--box-size) + var(--box-size) / 2`}}>
          <div className="now-line" style={{left: nowLineLeftProperty}} ref={nowRef}>
            <div className="now-line-notch"></div>
          </div>
